<template>
  <div class="min-h-screen">
    <!-- Loading -->
    <div v-if="loadingSalon" class="flex items-center justify-center min-h-screen">
    ...
    <template v-else>
      ...
      <div class="container-app -mt-16 relative z-10 pb-44">
        ...
        <div class="p-6 rounded-3xl bg-white border border-lilac-100 shadow-soft mb-6">
            ...
            <div class="flex-1">
              ...
              <div class="flex items-center gap-3 mt-2">
                <div class="flex items-center gap-1">
                  ...
                  <span class="text-xs text-gray-400">({{ salon.reviewCount }} avaliacoes)</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div v-if="salon.address" class="flex items-center gap-2 text-sm text-gray-500">
        ...
        <div class="grid grid-cols-4 gap-3 mb-6">
          <a v-if="salon.phone" :href="`tel:${salon.phone}`" class="p-4 rounded-2xl bg-white border border-lilac-100 hover:border-lilac-200 transition-all shadow-soft flex flex-col items-center gap-2">
          ...
          <button v-else class="p-4 rounded-2xl bg-white border border-lilac-100 shadow-soft flex flex-col items-center gap-2 opacity-50 cursor-not-allowed">
            <div class="w-10 h-10 rounded-xl bg-lilac-100 flex items-center justify-center">
              <Icon name="lucide:phone" class="w-5 h-5 text-lilac-600" />
            </div>
            <span class="text-xs text-gray-600">Ligar</span>
          ...
          <a
            v-if="salon.whatsapp"
            :href="`https://wa.me/55${salon.whatsapp.replace(/\D/g, '')}`"
            target="_blank"
            class="p-4 rounded-2xl bg-white border border-lilac-100 hover:border-lilac-200 transition-all shadow-soft flex flex-col items-center gap-2"
          >
        ...
        <div v-if="activeTab === 'services'" class="space-y-4">
          <div v-if="salon.services.length === 0" class="text-center py-12">
            <Icon name="lucide:sparkles" class="w-12 h-12 text-lilac-300 mx-auto mb-3" />
            <p class="text-gray-500">Nenhum servico disponivel</p>
          </div>
          <div
          ...
          >
            <div class="flex items-center justify-between">
              ...
              <div class="text-right">
                <p class="font-bold text-lilac-600">R$ {{ (service.price || 0).toFixed(2) }}</p>
                <button
                  class="mt-1 px-3 py-1 rounded-lg text-xs font-medium transition-all"
                ...
                >
                  {{ isServiceSelected(service.id) ? 'Selecionado' : 'Selecionar' }}
                </button>
              </div>
            </div>
          </div>
        ...
        <div v-if="activeTab === 'team'" class="grid grid-cols-2 gap-4">
          <div v-if="salon.team.length === 0" class="col-span-2 text-center py-12">
            <Icon name="lucide:users" class="w-12 h-12 text-lilac-300 mx-auto mb-3" />
            <p class="text-gray-500">Nenhum profissional listado</p>
          </div>
          <div
          ...
          >
            ...
            <div v-if="professional.rating" class="flex items-center justify-center gap-1 mt-2">
              <Icon name="lucide:star" class="w-4 h-4 text-rose-400 fill-current" />
        ...
        <div v-if="activeTab === 'reviews'" class="space-y-4">
          <div v-if="salon.reviews.length === 0" class="text-center py-12">
            <Icon name="lucide:message-square" class="w-12 h-12 text-lilac-300 mx-auto mb-3" />
        ...
        <div v-if="activeTab === 'info'" class="space-y-4">
          <div class="p-4 rounded-2xl bg-white border border-lilac-100 shadow-soft">
            <h3 class="font-semibold text-gray-800 mb-3">Horario de Funcionamento</h3>
            <div v-if="salon.workingHours.length > 0" class="space-y-2">
            ...
            <p v-else class="text-sm text-gray-500">Horarios nao informados</p>
          </div>
          ...
          <div class="p-4 rounded-2xl bg-white border border-lilac-100 shadow-soft">
            ...
            <div class="flex flex-wrap gap-2">
              ...
              <span class="px-3 py-1 rounded-lg bg-gray-100 text-sm text-gray-600">Cartao de Credito</span>
              <span class="px-3 py-1 rounded-lg bg-gray-100 text-sm text-gray-600">Cartao de Debito</span>
            </div>
          </div>
          ...
          <div class="p-4 rounded-2xl bg-white border border-lilac-100 shadow-soft">
            ...
            <div class="flex flex-wrap gap-2">
              ...
              <span class="px-3 py-1 rounded-lg bg-lilac-50 text-sm text-lilac-600">Cafe</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="fixed bottom-0 left-0 right-0 p-4 bg-white/90 backdrop-blur-xl border-t border-lilac-100 safe-bottom z-40">
        <div class="container-app">
          ...
          >
            ...
            Agendar Horario
            <span v-if="selectedServices.length > 0" class="ml-1 px-2 py-0.5 rounded-full bg-white/20 text-sm">
              {{ selectedServices.length }} servico(s)
            </span>
          </button>
        </div>
      </div>
      ...
      <UModal v-model="showBookingModal" :ui="{ width: 'max-w-lg' }">
        <div class="p-6">
          <h2 class="text-xl font-display font-semibold text-gray-800 mb-6">Agendar Horario</h2>

          <div class="space-y-4">
            <!-- Selected Services -->
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">Servicos selecionados</label>
              <div v-if="selectedServices.length > 0" class="space-y-2">
                ...
                >
                  ...
                  <div class="flex items-center gap-2">
                    <span class="font-medium text-lilac-600">R$ {{ (service.price || 0).toFixed(2) }}</span>
                    <button @click="selectService(service)" class="p-1 rounded hover:bg-lilac-100 transition-colors">
                      <Icon name="lucide:x" class="w-4 h-4 text-gray-400" />
                    </button>
                  </div>
                </div>
              </div>
              <div v-else class="p-4 rounded-xl bg-gray-50 text-center text-sm text-gray-500">
                Selecione servicos na aba "Servicos" antes de agendar
              </div>
            </div>
            ...
            <div>
              ...
              >
                ...
                <option v-for="pro in salon.team" :key="pro.id" :value="pro.id">{{ pro.name }}</option>
              </select>
            </div>

            <!-- Client Name (for guests) -->
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">Seu nome</label>
              <input
                type="text"
                v-model="bookingClientName"
            ...
            <button
              ...
              :disabled="bookingSubmitting || selectedServices.length === 0 || !bookingDate || !selectedTime || !bookingClientName"
              class="w-full py-4 rounded-xl bg-gradient-to-r from-lilac-500 to-rose-500 text-white font-semibold hover:from-lilac-600 hover:to-rose-600 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
      ...
      <div v-if="showSuccessModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4" @click.self="showSuccessModal = false">
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-sm p-8 text-center">
          ...
          <h3 class="text-xl font-display font-semibold text-gray-800 mb-2">Agendamento Confirmado!</h3>
          <p class="text-gray-500 mb-6">Seu horario foi agendado com sucesso. Voce recebera uma confirmacao em breve.</p>
          <button
...
const bookingClientName = ref('')
const bookingClientPhone = ref('')
...
// Dados do salao (usa dados da API ou fallback)
const salon = computed(() => {
  if (salonData.value) {
    return {
      ...
      category: salonData.value.category || 'Salao de Beleza',
  ...
  return {
    id: '',
    slug: slug,
    name: 'Salao nao encontrado',
    category: '',
    logo: '',
    coverImage: '',
    description: 'Este salao nao foi encontrado.',
    address: '',
    rating: 0,
    reviewCount: 0,
    distance: '',
...
const openMaps = () => {
  if (salon.value.address) {
    window.open(`https://maps.google.com/?q=${encodeURIComponent(salon.value.address)}`, '_blank')
  }
}

const confirmBooking = async () => {
  if (!salon.value.id || selectedServices.length === 0 || !bookingDate.value || !selectedTime.value || !bookingClientName.value) {
    return
  }
  ...
  try {
    ...
    for (const service of selectedServices.value) {
      await $fetch('/api/painel/appointments', {
        ...
        body: {
          ...
          professionalId: selectedProfessionalId.value || null,
          date: bookingDate.value,
          startTime: selectedTime.value,
          total: service.price || 0,
          clientName: bookingClientName.value,
          clientPhone: bookingClientPhone.value,
          status: 'pending',
          source: 'booking-page'
        }
      })
    ...
    bookingDate.value = ''
    bookingClientName.value = ''
    bookingClientPhone.value = ''
  } catch (error) {
